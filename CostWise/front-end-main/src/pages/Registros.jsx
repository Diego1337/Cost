...
const Registros = ({ records: allRecords, onSave, onDelete, showNotification, onDataRefresh }) => {
    ...
    const fetchMateriais = async () => {
        try {
            ...
            console.log("Chegou aqui! Dados recebidos:", data); // AQUI! Dentro da função
        } catch (error) {
    ...
    const confirmDelete = () => {
        const idToDelete = activeTab === 'Produtos' ? modalContent.id_prdt : modalContent.id;
    ...
    const renderTableContent = () => {
        if (!filteredRecords || filteredRecords.length === 0) {
            return <tr><td colSpan="6" style={{ textAlign: 'center', padding: '2rem' }}>Nenhum resultado encontrado.</td></tr>;
        }
